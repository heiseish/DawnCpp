...
public:
    ...
    constexpr static const std::string_view& BinanceHost =
    ...
    static size_t CurlCb(void* content, size_t size, size_t nmemb,
                         std::string* buffer);
    ...
    std::string GetAggTrades(const std::string_view& symbol, const int& fromId,
    ...
    std::string GetKLines(const std::string_view& symbol,
                          const std::string_view& interval, const int& limit,
                          const time_t& startTime, const time_t& endTime);

    // API + Secret keys required
    std::string GetAccount(const long& recvWindow);

    std::string GetMyTrades(const std::string_view& symbol, const int& limit,
                            const long& fromId, const long& recvWindow);
    ...
    std::string GetOrder(const std::string_view& symbol, const long& orderId,
                         const std::string_view& origClientOrderId,
                         const long& recvWindow);

    std::string CancelOrder(const std::string_view& symbol, const long& orderId,
                            const std::string_view& origClientOrderId,
                            const std::string_view& newClientOrderId,
                            const long& recvWindow);
    ...
    std::string Withdraw(const std::string_view& asset,
                         ...
                         const double& amount, const std::string_view& name,
                         const long& recvWindow);

    std::string GetDepositHistory(const std::string_view& asset,
                                  const int& status, const long& startTime,
                                  const long& endTime, const long& recvWindow);
